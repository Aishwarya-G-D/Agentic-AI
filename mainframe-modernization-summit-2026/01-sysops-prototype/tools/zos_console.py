"""z/OS Console Commands Tool — Group 2 SysOps Foundation.

This tool sends display-only commands to z/OS via the z/OSMF REST Console API
...
def _run_console_command(command: str, max_retries: int = 10) -> dict:
    ...
    host = connections.key_value("group_2_mainframe_connection")["host"]
    port = connections.key_value("group_2_mainframe_connection")["port"]
    credentials = {
        "user": connections.key_value("group_2_mainframe_connection")["username"],
        "password": connections.key_value("group_2_mainframe_connection")["password"],
    }
...
@tool(
    expected_credentials=[
        ExpectedCredentials(
            app_id="group_2_mainframe_connection",
            type=ConnectionType.KEY_VALUE,
        )
    ]
)
def group_2_display_active_jobs() -> dict:
...
def group_2_display_address_space(jobname: str) -> dict:
...
def group_2_display_spool_utilization() -> dict:
...
def group_2_display_wtor_messages() -> dict:
...
def group_2_display_iplinfo() -> dict:
...
def group_2_display_online_dasd() -> dict:
